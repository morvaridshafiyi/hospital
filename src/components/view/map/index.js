...
const Map = ({locations}) => {
  ...
  const markers = locations.map((item, index) => ({position: item, popupText: `Marker ${index + 1}`}))
  ...
  //   { position: [35.835298, 52.104343], popupText: "Marker 4" },
  // ];
  // const an = locations.map((item) => console.log(item, "rerer"))
